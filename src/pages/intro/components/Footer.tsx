...
export function Footer() {
  const handleStartNow = useStartNowAction('footer');

  const handleCommunication = () => {
    GA4Events.pageView('/footer/communication', '푸터 - 운영진과의 소통 이동');
    window.open('http://pf.kakao.com/_TwEUn/chat', '_blank', 'noopener,noreferrer');
  };

  const handleNotice = () => {
    GA4Events.pageView('/footer/notices', '푸터 - 공지사항 이동');
    window.open('https://repik-help.notion.site/', '_blank', 'noopener,noreferrer');
  };

  const handleTerms = () => {
    GA4Events.pageView('/footer/terms', '푸터 - 이용약관 이동');
    window.open('https://repik-help.notion.site/terms-of-services', '_blank', 'noopener,noreferrer');
  };

  const handlePrivacy = () => {
    GA4Events.pageView('/footer/privacy', '푸터 - 개인정보처리방침 이동');
    window.open('https://repik-help.notion.site/privacy-policy', '_blank', 'noopener,noreferrer');
  };

  const handleBusinessInfo = () => {
    GA4Events.pageView('/footer/business-info', '푸터 - 사업자정보 확인 시도');
    alert('서비스 준비 중입니다.');
  };

  const handleInstagram = () => {
    GA4Events.pageView('/footer/instagram', '푸터 - 인스타그램 링크 클릭');
    alert('서비스 준비 중입니다.');
  };
      ...
      <div className={cx('container')}>
        <div className={cx('cta')}>
          ...
          <p className={cx('ctaDescription')}>
            Repik과 함께 특가할인 쇼핑의 새로운 시대를 경험하세요
          </p>
          <button
            type="button"
            className={cx('downloadButton')}
            onClick={handleStartNow}
            data-testid="button-start-now-footer"
          >
            지금 시작하기
          </button>
        </div>
        ...
        <div className={cx('footerContent')}>
          ...
          <div className={cx('bottomBar')}>
            <p className={cx('copyright')}>
              © 2025 Repik. All rights reserved.
            </p>
            <div className={cx('socialLinks')}>
              <button
                type="button"
                className={cx('socialLinkButton')}
